/* ============================================================
   ...
   MICRO-BATCHED FREE-TIER OSINT ENRICHMENT PIPELINE (FIXED)
   ============================================================ */
...
async function main() {
  ...
  // Filter valid un-enriched candidates first, then strictly slice to max 10 per run
  const allCandidates = companies.filter((c) => {
  ...
  console.log(`[Micro-Batch] Processing ${candidates.length} leads out of ${allCandidates.length} pending...`);
