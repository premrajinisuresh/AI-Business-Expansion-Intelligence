...
async function main() {
  ...
  for (const company of data.companies) {
    ...
    if (clearIfInvalid(company, "Address", isValidAddress)) { fieldsCleared++; touchedThisCompany = true; }
    if (clearIfInvalid(company, "PIN", isValidPin)) { fieldsCleared++; touchedThisCompany = true; }
    ...
    // known Tamil Nadu city paired with an unrelated state) means
    // the State was almost certainly scraped from the wrong place.
    if (hasValue(company.State) && !isValidState(company.State, company.City)) {
      company.State = "";
      fieldsCleared++;
      touchedThisCompany = true;
    }
    if (hasValue(company.City) && !isValidCity(company.City)) {
      company.City = "";
      fieldsCleared++;
      touchedThisCompany = true;
    }
