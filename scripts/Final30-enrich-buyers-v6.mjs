/* ============================================================
   ...
   RATE-LIMIT RESILIENT GEMINI OSINT ENRICHMENT PIPELINE
   ============================================================ */
...
const BATCH_SIZE = 2; // Reduced batch size to respect rate limits
const BATCH_DELAY_MS = 4000; // Safe inter-batch delay
  ...
  for (let attempt = 1; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if (response.status === 429) {
        console.warn(`[Rate Limit 429] Encountered quota limit in enrichment. Retrying in ${backoff / 1000}s (Attempt ${attempt}/${retries})...`);
        await delay(backoff);
...
async function main() {
  ...
  console.log(`Starting rate-limit resilient enrichment pipeline for ${candidates.length} candidates...`);
  ...
  for (let i = 0; i < candidates.length; i += BATCH_SIZE) {
    ...
    for (const company of batch) {
      ...
      stats.processed++;
      await delay(2000); // Delay between individual requests inside a batch
    }
