...
const PLACEHOLDER_LOCAL_PARTS = new Set([
  "abc", "test", "xxx", "sample", "demo", "dummy", "placeholder",
  "yourname", "username", "noreply", "donotreply"
]);
...
export function isValidEmail(value) {
  ...
  // TLD capped to 2-10 letters: real TLDs are almost never longer
  // than that. This is also what catches two emails that got glued
  // together with no separator between them (e.g.
  // "name@gmail.comotherinfo") - a real bug seen in production
  // where two adjacent page elements had no whitespace between
  // them, so their text ran together during scraping.
  if (!/^[^\s@]+@[^\s@]+\.[a-z]{2,10}$/i.test(v)) return false;
  if (/\.(png|jpe?g|gif|svg|webp|bmp)$/i.test(v)) return false;
  const [localPart, domain] = v.split("@");
  // A single-character local part (e.g. "o@company.com") is almost
  // always a truncation artifact (likely meant to be "info@...").
  if (!localPart || localPart.replace(/[-._+]/g, "").length < 2) return false;
  const normalizedLocal = localPart.replace(/[^a-z0-9]/g, "");
  if (PLACEHOLDER_LOCAL_PARTS.has(normalizedLocal)) return false;
