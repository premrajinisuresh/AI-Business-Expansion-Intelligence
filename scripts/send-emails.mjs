...
const CATEGORY_RULES = [
  ...
  // ---- Added so every one of the 13 real auto-search categories
  // gets a properly targeted pitch instead of falling through to
  ...
  {
    key: "institutionalconsultants",
    match: [
      "institutional property", "property consultant", "institutional real estate",
      "hospital consultant", "college consultant", "campus developer",
      "healthcare infrastructure", "education infrastructure"
    ],
    subject: (c) => `Institutional Land Opportunity for ${c.Company} — Madurai Highway Junction`,
    pitch:
      "As a consultant connecting hospitals, colleges, and institutional buyers with the right sites, this corner commercial plot at a major Madurai highway junction offers strong road frontage and visibility — well suited to recommend to your healthcare, education, or hospitality clients scouting land in this region."
  }
];
...
async function processCompany(company, data, stats) {
  ...
  try {
    ...
    // Only upgrade Status from "New" -> "Contacted". Never downgrade
    // or overwrite a status you've since set manually (Interested,
    // Not Interested, Closed, etc.) - this is what makes the sent
    // email actually show up in the dashboard's counts/filter.
