/* ============================================================
   Enrichment Script
   UPDATED: Integrated multi-directory hunting module (Google Business Profile,
   JustDial, IndiaMART, Sulekha, TradeIndia) and automated WhatsApp fallback.
   ============================================================ */
...
// MULTI-DIRECTORY PHONE HUNT MODULE
async function huntPhoneNumberViaSearch(company) {
  ...
  const directoryQueries = [
    `"${company.Company}" google business profile phone whatsapp contact ${location} India`,
    `"${company.Company}" phone number contact justdial ${location} India`,
    `"${company.Company}" phone number contact indiamart ${location} India`,
    `"${company.Company}" phone number contact sulekha ${location} India`,
    `"${company.Company}" phone number contact tradeindia yellowpages ${location} India`,
    `"${company.Company}" general contact phone whatsapp ${location} India`
  ];

  for (const query of directoryQueries) {
    const phone = await runPhoneSearchPass(company, query);
...
async function processCompany(company, stats) {
  ...
  // Multi-Directory Phone Hunt Fallback & WhatsApp Auto-Population
  const stillNoNumber =
    ...
    if (found) {
      ...
      if (/^[6-9]/.test(last)) {
        ...
        if (isValidMobile(candidate)) {
          if (isEmptyValue(company.Mobile)) {
            company.Mobile = candidate;
            wasUpdated = true;
          }
          if (isEmptyValue(company.WhatsApp) && isValidWhatsApp(candidate)) {
