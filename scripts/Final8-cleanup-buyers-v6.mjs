...
async function main() {
  ...
  for (const company of data.companies) {
    ...
    if (clearIfInvalid(company, "Email", isValidEmail)) {
      ...
      // The bad email may have already been "sent" (and bounced,
      // or never delivered). Reset send-tracking so that once a
      // correct email is found by a future Enrich run, it will
      // actually be emailed instead of being skipped forever as
      // "already sent".
