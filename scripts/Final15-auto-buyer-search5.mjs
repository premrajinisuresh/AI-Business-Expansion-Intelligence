...
async function extractLeadsWithGemini(category, searchResults) {
  ...
  const prompt =
    ...
    `Company, Website, Email, Phone, ContactPerson, Notes, SourceURL. (A separate enrichment ` +
    `step will visit each website directly afterward to fill in real contact details, so it's ` +
    `fine to leave Email/Phone as "Not public" for now if not clearly present.)\n\n` +
