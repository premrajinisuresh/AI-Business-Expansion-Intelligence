...
async function extractLeadsWithGemini(category, searchResults) {
  ...
  const prompt =
    ...
    `aggregators.${consultantNote} REQUIRED: only include an organization if you can find its ` +
    `actual official website URL in the search results above — if no real website is present ` +
    `for an organization, leave it out entirely, even if the organization itself sounds like a ` +
    `good fit (a company with no website can never be contacted for a phone/WhatsApp number by ` +
    `the enrichment step, so it's better skipped than saved empty). For each organization, output ` +
    `an object with fields: Company, Website, Email, Phone, ContactPerson, Notes, SourceURL. Use ` +
    `the exact string "Not public" only for Email/Phone/ContactPerson/Notes fields that are not ` +
    `clearly present — never invent an email or phone number, and never leave Website blank or ` +
    `"Not public". (A separate enrichment step will visit each website directly afterward to fill ` +
    `in real contact details, so it's fine to leave Email/Phone as "Not public" for now.)\n\n` +
