...
async function extractLeadsWithGemini(category, searchResults) {
  ...
  const prompt =
    ...
    `(A separate enrichment step will visit each website directly afterward to fill in real ` +
    `contact details, so it's fine to leave these as "Not public" for now.)\n\n` +
